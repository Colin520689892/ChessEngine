...
void Zobrist::add(int depth, int score, Flag flag) {//新增条目
	...
	//如果已有深度,且深度更大,就不添加
	if (tempItem->flag != EMPTY && tempItem->depth > depth)return;
...
int Zobrist::get(int depth, int alpha, int beta) {//获取条目
	...
	if (res == EMPTY)return UNKNOWN_SCORE;
	if (tempItem->ZVal == CurrentZVal && tempItem->depth >= depth) {
		if (res == EXACT)return tempItem->score;
		if (res == ALPHA && tempItem->score <= alpha)return alpha;
		if (res == BETA && tempItem->score >= beta)return beta;
	}
